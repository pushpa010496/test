          ...
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
              <li class="breadcrumb-item">
                <a href="{{ url('/editorialsection') }}">Advertise Section</a>
              </li>
            ...
            <div class="col-lg-9 col-md-9 col-sm-9 col-12 PF-BrdrDDD p-1">
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 col-12 title-list">
               <h2 class="mt-3 mb-0 PF-TXTBlk000">
                   @if($current_issue->id === $healthcareinfo->issue_id)
                    <a href="{{url('latest-ebook')}}" title="{{$healthcareinfo->title}}" class="PF-TXTBlk000">{{$healthcareinfo->title}}</a>
                   @else
              ...
              {{--   <p class="PF-TXTBlk pt-1">{{$healthcareinfo->short_description}}...</p>  --}}
                 ...
                 <p class="m-0 text-justify text-dark">{{ str_limit(preg_replace('/[^a-zA-Z\s]/', '', strip_tags(html_entity_decode(ucfirst($healthcareinfo->short_description)))),$limit = 350, $end = '...') }}</p>           
              </div> 
              @endforeach
